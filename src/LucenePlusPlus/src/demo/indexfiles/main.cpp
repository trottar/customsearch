...
DocumentPtr fileDocument(const String& docFile) {
    ...
    doc->add(newLucene<Field>(L"path", docFile, Field::STORE_YES, Field::INDEX_ANALYZED));
    ...
    //doc->add(newLucene<Field>(L"modified", DateTools::timeToString(FileUtils::fileModified(docFile), DateTools::RESOLUTION_MINUTE),Field::STORE_YES, Field::INDEX_NOT_ANALYZED));
    doc->add(newLucene<Field>(L"modified", DateTools::timeToString(FileUtils::fileModified(docFile), DateTools::RESOLUTION_MINUTE),Field::STORE_YES, Field::INDEX_ANALYZED));
...
int main(int argc, char* argv[]) {
	...
	std::wcout << L"MaxFieldLength: " << writer->getMaxFieldLength() << L"\n";
	writer->setMaxFieldLength(784662528);
	std::wcout << L"MaxFieldLength: " << writer->getMaxFieldLength() << L"\n";
